...
AjaxSolr.AbstractFacetWidget = AjaxSolr.AbstractWidget.extend(
   ...
   * @param {String} field The field specified.
   * @param {Manager} manager the manager to handle current search.
   * @returns {Function} Sends a request to Solr if it successfully adds a
   *   filter query with the given value.
   */
  clickHandler: function (value, field, manager) {
    var self = this, meth = this.multivalue ? 'add' : 'set';
    return function () {
      if (!self.field){
        self.field = field;
      }
      manager.curr[field][value] = value;
      if (self[meth].call(self, value)) {
        self.doRequest(null, "select",true);
      }
      else{
        console.log("else statement, [DEBUG]");
        self.doRequest(null,"select",true);
      }
      return false;
    }
